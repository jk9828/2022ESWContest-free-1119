...
def get_Yolov5_weight():
    ...
    model = torch.hub.load('/home/pi/Desktop/love/yolov5-master', 'custom', path='/home/pi/Desktop/love/best (1).pt', source='local')
    model.eval()
...
def include(list1, desire_num,finalbusnum):
    if desire_num in list1:
         string2voice(desire_num,finalbusnum)
    else:
        return 0
...
def string2voice(desire_num,finalbusnum): #원하는 버스의 번호와 거리 정보를 입력받아 음성으로 출력해주는 함수
    ...
    global time_dummy_detected #함수 밖에 있는 임시 시간 저장 변수를 전역변수로 지정
    time_dummy_detected = time.time() #음성이 출력될 때의 시간을 마이크로 단위로 저장
...
def cpu1(desire_num_list):#공유 리스트 desire_num_list는 [셀레니움에서 받은 값, 처리 가능 여부 숫자1, 처리 가능 여부 숫자2] 형태
    ...
    for i, img_file in enumerate(img_listed):
        
        text1=img2num(img_file)
        busnum_text.append(text1)
        print(text1)
        
    final_busnum = []

    for busnumtext in busnum_text:    
        try:
            numtext = re.findall("\d+", busnumtext)
        
            final_busnum = final_busnum+numtext
        except:
            pass
        
    if desire_num_list[0] in final_busnum:
        string2voice(desire_num_list[0],final_busnum)#버스 번호와 최종 버스 목록을 인자로 받음
        clear_selenium()
        print("done")
...
def cpu2(desire_num_list):
    ...
    time_gap=10 #음성이 켜지고 LCD가 꺼질 때까지의 시간 간격 설정
    global time_dummy_detected
    if (time.time()-time_dummy_detected>time_gap): #음성이 켜졌을 때 시간과 비교하여 일정 시간이 지나면 LCD를 끔
        bus_list_LCD_OFF() #LCD를 끄는 함수
        time_dummy_detected=10000*1000
...
if __name__ == '__main__':
    ...
    desire_num_list = manager.list(range(3)) #공유 리스트 desire_num_list를 선언
    ...
    #cap = cv2.VideoCapture(0) #영상을 입력받을 카메라 디바이스 객체 생성
    #cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1902) # 가로
    #cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 1080)
    #time_dummy_detected=10000*100 #gTTs가 켜질 때의 시간을 저장할 변수를 큰 값으로 초기화 
    model = get_Yolov5_weight()
    i = 0

    while True:
        cam_capture()
        #무한으로 이 과정을 일정 시간마다 반복
        #cpu1, cpu2 함수를 각 프로세스에서
        #공유 리스트 desire_num_list 를 인자로 받아 실행
        time_dummy_detected=10000*100
        ...
        i = i +1
        print(i)
        proc1 = multiprocessing.Process(target=cpu1,args=(desire_num_list,))
        proc2 = multiprocessing.Process(target=cpu2,args=(desire_num_list,))
        if(desire_num_list[1]==1): #공유 리스트에 시작 가능 변수가 1일때 프로세스 실행
            proc1.start() #각 프로세스를 실행
